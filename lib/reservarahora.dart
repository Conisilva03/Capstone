...
class _ReservarAhoraScreenState extends State<ReservarAhoraScreen> {
  ...
  void initState() {
    ...
    selectedTime =
        TimeOfDay(hour: 8, minute: 0); // Set the start time to 8:00 AM
    horaController.text =
  ...
  Future<String> checkReservationInRange(
      String spotId, String startDateTime, String endDateTime) async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            'https://api2.parkingtalcahuano.cl/reservations/check?spot_id=$spotId'),
        headers: {
          'Content-Type': 'application/json',
        },
...
// Utility function to calculate the reservation cost based on minutes
  String calculateReservationCost(
      DateTime start, DateTime end, int ratePerMinute) {
  ...
  Future<void> _submitForm() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (selectedDate == null ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Seleccione una fecha y hora válidas.'),
          ),
        );
        return;
      }

      // Convertir los objetos TimeOfDay a objetos DateTime para comparar los tiempos
      ...
      // Verificar si la hora de fin es mayor a la hora de inicio
      if (!endTime.isAfter(startTime)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Seleccione una hora de fin mayor a la hora de inicio.'),
          ),
        );
        return;
      }
      ...
      bool isBalanceSufficient =
          await checkWalletBalance(startDateTime, endDateTime);
      ...
      final String reservationStatus =
          await checkReservationInRange(widget.id, startDateTime, endDateTime);
            ...
            builder: (context) => ConfirmationScreen(
              ...
              onAccept: () async {
                ...
                await sendParkingMovementData(
                  ...
                  double.parse(
                      reservationCost), // Assuming cost is the total_cost
...
// Método para verificar si el dinero en la billetera es suficiente
  Future<bool> checkWalletBalance(
      String startDateTime, String endDateTime) async {
    try {
      // Fetch the user ID
      int? userId = await fetchUserId();
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Tu saldo en la billetera no es suficiente para realizar la reserva.'),
          actions: [
            TextButton(
              onPressed: () {
                ...
                if (parkingSpace != null)
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                          'Estado: ${parkingSpace!.state ? 'Disponible' : 'Ocupado'}'),
